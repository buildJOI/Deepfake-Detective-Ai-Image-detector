...
class AppSettings {
  ...
  static const String defaultUrl = 'https://deepfake-detective-ai-image-detector-production.up.railway.app'; // ← update this
...
// ─────────────────────────────────────────────────────────────────────────────
// Logo Widgets — using your logo asset
// ─────────────────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: kNeon.withOpacity(glowOpacity), blurRadius: 40, spreadRadius: 10),
        ],
      ),
...
class _SplashScreenState extends State<SplashScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _glowCtrl = AnimationController(duration: const Duration(milliseconds: 1500), vsync: this)
      ..repeat(reverse: true);
        ...
        child: FadeTransition(
          ...
          child: Column(
            ...
            children: [
              AnimatedBuilder(
                animation: _glowAnim,
                builder: (_, __) => AppLogo(size: 100, glow: true, glowOpacity: _glowAnim.value * 0.6),
              ),
              const SizedBox(height: 32),
              ShaderMask(
                shaderCallback: (b) => const LinearGradient(colors: [kNeon, kNeonPurple]).createShader(b),
                child: const Text('SusAI',
                  style: TextStyle(fontSize: 52, fontWeight: FontWeight.w900, letterSpacing: 6, color: Colors.white)),
              ),
              const Text('AI MEDIA DETECTOR',
                style: TextStyle(fontSize: 12, letterSpacing: 6, color: kSubtext)),
              const SizedBox(height: 48),
              SizedBox(width: 120,
                child: LinearProgressIndicator(
                  backgroundColor: kSurface,
                  valueColor: const AlwaysStoppedAnimation<Color>(kNeon))),
              const SizedBox(height: 16),
              const Text('INITIALIZING AI ENGINE...',
                style: TextStyle(fontSize: 10, letterSpacing: 3, color: kSubtext)),
            ],
          ),
        ),
      ),
...
class _HomeScreenState extends State<HomeScreen> {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Row(children: [
                    ...
                    ShaderMask(
                      shaderCallback: (b) => const LinearGradient(colors: [kNeon, kNeonPurple]).createShader(b),
                      child: const Text('SusAI',
                        style: TextStyle(fontSize: 26, fontWeight: FontWeight.w900, letterSpacing: 3, color: Colors.white)),
                    ),
                  ]),
                  IconButton(
              ...
              const Text('AI-powered media authenticity detection',
                style: TextStyle(color: kSubtext, fontSize: 12, letterSpacing: 1)),
              const SizedBox(height: 40),

              _menuCard(context, icon: Icons.image_search, title: 'IMAGE SCAN',
                subtitle: 'Detect AI-generated images', color: kNeon,
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ImageScanScreen()))),
              const SizedBox(height: 16),
              _menuCard(context, icon: Icons.videocam_outlined, title: 'VIDEO SCAN',
                subtitle: 'Analyze video frame by frame', color: kNeonPurple,
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const VideoScanScreen()))),
              const SizedBox(height: 16),
              _menuCard(context, icon: Icons.history, title: 'SCAN HISTORY',
                subtitle: 'View previous detections', color: kNeonGreen,
  ...
  Widget _menuCard(BuildContext context, {required IconData icon, required String title,
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _test() async {
    ...
    try {
      ...
      setState(() {
        _testSuccess = r.statusCode == 200;
        _testResult = r.statusCode == 200 ? '✓ Connected successfully!' : '✗ Server error ${r.statusCode}';
      });
    } catch (_) {
      setState(() { _testSuccess = false; _testResult = '✗ Cannot reach server. Check the URL.'; });
  ...
  Future<void> _save() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Settings saved!'), backgroundColor: kNeonGreen));
      ...
      body: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(12),
                borderSide: BorderSide(color: kNeonPurple.withOpacity(0.3))),
          ...
          Container(
            ...
            decoration: BoxDecoration(color: kNeon.withOpacity(0.05), borderRadius: BorderRadius.circular(10),
          ...
          SizedBox(width: double.infinity, height: 48,
            child: OutlinedButton(
              ...
              style: OutlinedButton.styleFrom(side: const BorderSide(color: kNeon),
          ...
          if (_testResult != null) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: (_testSuccess! ? kNeonGreen : kNeonRed).withOpacity(0.08),
                borderRadius: BorderRadius.circular(10),
                border: Border.all(color: (_testSuccess! ? kNeonGreen : kNeonRed).withOpacity(0.4))),
          ...
          SizedBox(width: double.infinity, height: 54,
            child: ElevatedButton(
              onPressed: _save,
              style: ElevatedButton.styleFrom(backgroundColor: kNeonPurple, foregroundColor: Colors.white,
  ...
  Future<void> _analyze() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        scanHistory.insert(0, ScanRecord(
          filename: data['filename'] ?? 'image', label: data['label'],
          confidence: (data['confidence'] as num).toDouble(), type: 'image', timestamp: DateTime.now()));
      } else { setState(() => _error = 'Server error: ${res.statusCode}'); }
    } catch (_) { setState(() => _error = 'Connection failed. Go to Settings ⚙ and check the URL.'); }
        ...
        child: Column(children: [
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: kCard, borderRadius: BorderRadius.circular(16),
                border: Border.all(color: _img != null ? kNeon.withOpacity(0.5) : kSubtext.withOpacity(0.2))),
              child: _img != null
                  ...
                  : Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      const Text('TAP TO SELECT IMAGE',
                        style: TextStyle(color: kSubtext, letterSpacing: 2, fontSize: 12)),
                    ]),
            ),
          ),
          const SizedBox(height: 20),
          Row(children: [
            Expanded(child: _outlineBtn(icon: Icons.photo_library, label: 'GALLERY', color: kNeon,
              onTap: () => _pick(ImageSource.gallery))),
            const SizedBox(width: 12),
            Expanded(child: _outlineBtn(icon: Icons.camera_alt, label: 'CAMERA', color: kNeonPurple,
          ...
          SizedBox(width: double.infinity, height: 54,
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(backgroundColor: kNeon, foregroundColor: kBg,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0),
              child: _loading
    ...
    showModalBottomSheet(context: context, backgroundColor: kCard,
      ...
      builder: (_) => Padding(padding: const EdgeInsets.all(24),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ListTile(leading: const Icon(Icons.photo_library, color: kNeon),
            title: const Text('Gallery', style: TextStyle(color: kText)),
            onTap: () { Navigator.pop(context); _pick(ImageSource.gallery); }),
          ListTile(leading: const Icon(Icons.camera_alt, color: kNeonPurple),
  ...
  Future<void> _analyze() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        setState(() => _result = data);
        scanHistory.insert(0, ScanRecord(
          filename: data['filename'] ?? 'video', label: data['verdict'],
          confidence: (data['avg_fake_confidence'] as num).toDouble(), type: 'video', timestamp: DateTime.now()));
      } else { setState(() => _error = 'Server error: ${res.statusCode}'); }
    } catch (_) { setState(() => _error = 'Connection failed. Go to Settings ⚙ and check the URL.'); }
    finally { setState(() => _loading = false); }
  }
        ...
        child: Column(children: [
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: kCard, borderRadius: BorderRadius.circular(16),
                border: Border.all(color: _video != null ? kNeonPurple.withOpacity(0.5) : kSubtext.withOpacity(0.2))),
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(_video != null ? Icons.video_file : Icons.video_library_outlined,
                  size: 56, color: kNeonPurple.withOpacity(_video != null ? 1.0 : 0.5)),
                const SizedBox(height: 12),
                Text(_video != null ? _video!.path.split('/').last : 'TAP TO SELECT VIDEO',
                  style: TextStyle(color: _video != null ? kText : kSubtext, letterSpacing: 1, fontSize: 12),
                  textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis),
                if (_video != null) ...[
                  const SizedBox(height: 8),
                  Text('${(_video!.lengthSync() / 1024 / 1024).toStringAsFixed(1)} MB',
          ...
          Container(
            ...
            decoration: BoxDecoration(color: kNeonPurple.withOpacity(0.05), borderRadius: BorderRadius.circular(12),
              border: Border.all(color: kNeonPurple.withOpacity(0.2))),
            child: Row(children: [
              Icon(Icons.info_outline, color: kNeonPurple.withOpacity(0.7), size: 18),
              const SizedBox(width: 12),
              const Expanded(child: Text('Analyzes up to 20 frames per video. Larger files may take longer.',
    ...
    return Scaffold(
      ...
      body: scanHistory.isEmpty
          ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              const SizedBox(height: 8),
              const Text('Run an image or video scan to see history',
              ...
              itemBuilder: (_, i) {
                ...
                return Container(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(r.filename, style: const TextStyle(color: kText, fontSize: 13, fontWeight: FontWeight.w600),
...
Widget _errorCard(String error) => Container(
  ...
  decoration: BoxDecoration(color: kNeonRed.withOpacity(0.08), borderRadius: BorderRadius.circular(12),
    border: Border.all(color: kNeonRed.withOpacity(0.4))),
  child: Row(children: [
...
Widget _resultCard(Map<String, dynamic> result) {
  ...
  return Container(
    ...
    child: Column(children: [
      ...
      const SizedBox(height: 12),
      Text(isFake ? 'AI GENERATED' : 'AUTHENTIC',
        style: TextStyle(color: color, fontSize: 22, fontWeight: FontWeight.w900, letterSpacing: 4)),
      const SizedBox(height: 20),
      ...
      ClipRRect(borderRadius: BorderRadius.circular(4),
        child: LinearProgressIndicator(value: conf / 100, backgroundColor: kSurface,
...
Widget _videoResultCard(Map<String, dynamic> result) {
  ...
  return Container(
    ...
    child: Column(children: [
      Icon(isFake ? Icons.warning_amber_rounded : Icons.verified_rounded, size: 52, color: color),
      const SizedBox(height: 12),
      Text(isFake ? 'DEEPFAKE DETECTED' : 'VIDEO AUTHENTIC',
...
Widget _confRow(String label, double value, Color color) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  const SizedBox(height: 4),
  ClipRRect(borderRadius: BorderRadius.circular(4),
    child: LinearProgressIndicator(value: value / 100, backgroundColor: kSurface,
...
Widget _statChip(String label, String value) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Text(value, style: const TextStyle(color: kSubtext, fontSize: 12, fontWeight: FontWeight.w600),
    maxLines: 1, overflow: TextOverflow.ellipsis),
]);